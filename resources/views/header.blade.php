    ...
    <div class="col-md-12">
        <div class="card card-primary">
            ...
            <div class="card-header navbar">
                ...
                <div class="btn-group me-2">
                    <a href="{{ route('media-index', ['path' => $url['path'], 'view' => 'table', 'select' => $select, 'close' => $close, 'CKEditorFuncNum' => $CKEditorFuncNum]) }}" class="btn @if($view == "table")btn-primary @else btn-light @endif"><i class="icon-list"></i></a>
                    <a href="{{ route('media-index', ['path' => $url['path'], 'view' => 'list', 'select' => $select, 'close' => $close, 'CKEditorFuncNum' => $CKEditorFuncNum]) }}" class="btn @if($view == "list")btn-primary @else btn-light @endif"><i class="icon-th"></i></a>
                </div>
            ...
            <div class="card-body">
                <nav class="breadcrumb-nav">
                    <ol class="breadcrumb" style="margin: -10px 0 6px 10px;">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('media-index') }}?select={{$select}}&CKEditorFuncNum={{$CKEditorFuncNum}}"><i class="icon-th-large"></i> </a></li>

                        @foreach($nav as $item)
                            <li class="breadcrumb-item"><a href="{{ $item['url'] }}&view={{$view}}&select={{$select}}&CKEditorFuncNum={{$CKEditorFuncNum}}"> {{ $item['name'] }}</a></li>
                        @endforeach
                    </ol>
                </nav>